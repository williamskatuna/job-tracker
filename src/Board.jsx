...
const onDragEnd = async (result) =>{
    ...
    setJobs(prev => prev.map(job =>
        job.id === draggableId
        ? { ...job, status: destination.draggableId}
        : job
    ...
    return (
        <div className="min-h-screen" style={{background: '#0d0d0d'}}>
            <nav style={{background:'#1a1a1a', borderBottom: '0.5 solid #2c333a'}} className="px-6 py-4 flex justify-between items-center">
                ...
                <button onClick={handleSignOut} className="text-sm px-3 py-1 rounded-lg" style={{color:'#f87171', border: '0.5px solid #6b2323', cursor: 'pointer'}}>
                    Sign Out
                </button>
                </div>
            </nav>
            <main  className="p-6">
                ...
                <button onClick={() => setShowModal (true)}
                    ...
                    style={{background: '#378add', color:'white', border:'none',cursor: 'pointer'}}> 
                    + Add Job
                </button>                  
                </div>
                ...
                <div className="grid grid-cols-4 gap-3 items-start">
                    ...
                    <Droppable droppableId={column} key={column}>
                        ...
                        <div className="flex justify-betweem items-center mb-3">
                            <h2 className="text-sm font-medium" style={{color: '#b6c2cf'}}>{column}</h2>
                            <span className="text-xs px-2 py-1 rounded-full" style={{background: '#2c333a', color: '#8c8bab'}}>
                        ...
                        //         onMoupseLeave={e => e.currentTarget.style.background='#2c333a'}                      
                        >
